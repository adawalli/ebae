...
- `filter=buyingOptions:{FIXED_PRICE}` when the search is BIN-only (per-search toggle; auctions can be included) - but sold tracking widens even a BIN-only poll to `{FIXED_PRICE|AUCTION}`, following those auctions for their close price without ever alerting on them (§3)
...
On a BIN-only search, sold tracking still surfaces auctions (it widens the poll filter, §2) and follows them silently: the winning bid joins the **same blended sold median** as BIN closes - an auction close is a realized value too - and no auction ever produces an alert. The query is the primary gate, so an auction whose current bid sits below the search's price floor is simply not surfaced, the same way the floor keeps sub-band accessories out of the market sample. One extra path drains the pre-feature backlog: a listing already in the seen set that is now running as an auction is normally passed over (dedupe filters it out of the alert loop), but if it carries an **alert row** - the user was told about it, as a BIN or before tracking widened the poll - it is re-followed for its close. The alert row is the discriminator that keeps the silent seed backlog and suppressed listings out; once followed (or already resolved) it is inert, so the scan's small extra read only fires while that backlog drains.

Four checks over 30 days leaves gaps a listing can sell inside and stop being readable before the next one - the realized price is simply lost. Quota that would expire at local midnight buys those checks early instead (`bonusBudget`, `runBonusChecks`). What it may spend is bounded twice: never into what the saved configuration still needs for the rest of the day plus a 5% headroom (which is what guarantees the governor below can't engage because of it), and never ahead of the elapsed pollable day, so the surplus trickles out all day rather than being dumped at once. One extra look per listing per day - a ledger the scheduled path shares, so a listing it just checked isn't asked again a few lines later in the same tick - at most 3 per tick, fixed-price only (an auction read before its end would look like "nobody bid"). It never spends one of the listing's four scheduled checks and never moves its schedule, except to take back a pending failure retry. Pacing is measured against the pollable time elapsed in the counter's own day (`counterDayFrac`), not the user's: the counter rolls on the server's calendar day while the snooze window is in the user's zone, and starting the clock at the user's midnight compares two different 24h windows - it charges spend to a day that has not begun, and withholds a surplus the hard bound says is free. These calls stay out of the projection on purpose: budgeting for them would engage the governor against the very thing that made them affordable. What they spend is also counted separately, in `api_usage.surplus` (a subset of `used`, flushed by the same upsert), so the quota tile can subtract it before judging whether the saved configuration is running ahead of pace - otherwise spending free quota would read as an overspend alarm.
