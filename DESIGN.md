...
- `q` = the user's search terms (plus optional `category_ids` and filters)
...
- `sort=newlyListed`, one maximum-size page (200 items). If eBay reports more matches, the Status page warns once for that truncation episode; a complete result rearms the warning.
